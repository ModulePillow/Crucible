...
public partial class CrucibleEngine
{
   ...
   void Resize()
   {
      ...
      PassResourceManager.Reinitialize(dx12Device, width, height);
   }
   ...
   void PopulateCommandList()
   {
      ...
      cmdList.Reset(frames[fence.FrameIndex].cmdAllocator, null);
      ...
      var rtvHandle = DescHeapManager.GetCPUHandle(frames[fence.FrameIndex].rtvIndex, ViewType.RTV);
      var dsvHandle = DescHeapManager.GetCPUHandle(FrameResource.dsvIndex, ViewType.DSV);
      cmdList.ClearRenderTargetView(rtvHandle, AppConstants.CleanColor);
      cmdList.ClearDepthStencilView(dsvHandle, ClearFlags.FlagsDepth, 1.0f, 0);
      PassResourceManager.CleanAll(cmdList);
      ...
      cmdList.PipelineState = PipelineStateManager.GetPipelineConfig("Default.hlsl").pso;
      cmdList.SetGraphicsRootSignature(PipelineStateManager.GetPipelineConfig("Default.hlsl").sign);
